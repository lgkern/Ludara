...
static void
loadTexture(char* filename, GLMmaterial* mat)
{
	mat->texType = -1;
	//printf ("\nLoading texture..\n");
	BITMAPINFO* info;
    ...
    if (info->bmiHeader.biHeight == 1)
      mat->texType = GL_TEXTURE_1D;
    else
      mat->texType = GL_TEXTURE_2D;
	...
	//tex->id = texture;
	mat->texId = texture;
	glBindTexture(mat->texType, mat->texId);
	...
	glTexParameteri(mat->texType, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	glTexParameteri(mat->texType, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	glTexParameteri(mat->texType, GL_TEXTURE_WRAP_S, GL_CLAMP);
    glTexParameteri(mat->texType, GL_TEXTURE_WRAP_T, GL_CLAMP);

    glTexImage2D(mat->texType, 0, 4, info->bmiHeader.biWidth, info->bmiHeader.biHeight,
...
glmReadMTL(GLMmodel* model, char* name)
{
    ...
    while(fscanf(file, "%s", buf) != EOF) {
		...
		case 'm':				/* map_Kd */
			...
			loadTexture(strdup(textureFilename), &(model->materials[nummaterials]));
...
glmDraw(GLMmodel* model, GLuint mode)
{
    ...
    while (group) {
        if (mode & GLM_MATERIAL) {
			...
			//printf("type: %d %d \n", material->texType, GL_TEXTURE_2D);
			//printf("id: %d \n", material->texId);

			if (material->texType == GL_TEXTURE_2D) {
				//printf("%s \n \n", material->texture->filename);
				//printf("type: %d %d \n", material->texture->type, GL_TEXTURE_2D);
				//printf("id: %d \n", material->texture->id);
				glEnable(GL_TEXTURE_2D);
				glTexEnvi(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE);
				glBindTexture(material->texType, material->texId);
			}
